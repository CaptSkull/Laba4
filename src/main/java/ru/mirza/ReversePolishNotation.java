...
class ReversePolishNotation {
        ...
        for (int i = 0; i < tokens.size(); i++) {
            switch (tokens.get(i).getType()) {
                ...
                case "VAR":
                    ...
                    i = hashSetToRPN(i,tokens,output);         //обрабатываем хэшсет и заносим в стек и преобразуем к ReversePolishNotation парсим все контейнеры add remove
                    break;

                case "WHILE":
                    i = whileToRPN(i, tokens, output);
                    break;
                case "LBRACE":
    ...
    private int hashSetToRPN(int i, List<Token> tokens, List<Token> output)  {
        label1:
    ...
    private int whileToRPN (int i, List<Token> tokens, List<Token> output) {
