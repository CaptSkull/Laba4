...
class BakaLinkedList {
    ...
    private class myNode {
        ...
        int kawaiiHashCodeList(String t, ArrayList<BakaLinkedList> hashSetTable) {
    ...
    void remove(String t, int hash, ArrayList<BakaLinkedList> hashSetTable) {
        for (myNode x = first; x != null; x = x.next) {
            if (hash == x.kawaiiHashCodeList(x.item, hashSetTable)) {
                if (t.equals(x.item)) {
    ...
    boolean contains(String t, int hash, ArrayList<BakaLinkedList> hashSetTable) {
    ...
    private int indexOf(String t, int hash, ArrayList<BakaLinkedList> hashSetTable) {
        int index = 0;
        for (myNode x = first; x != null; x = x.next) {
            if (hash == x.kawaiiHashCodeList(x.item, hashSetTable)) {
                if (t.equals(x.item))
